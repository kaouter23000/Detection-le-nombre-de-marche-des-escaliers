...
std::vector<Point> unification(std::vector<Point> p, int x, int y) {
    for (int i = 0; i < p.size(); i++)
        if (i > 0) 
            if (abs(p[i].x - p[i - 1].x) < x || abs(p[i].y - p[i - 1].y) < y) 
                p.erase(p.begin() + i);
    return p;
}
...
int main() {
    ...
    src = imread("img/escalier 3.png", 1);
    ...
    HoughLinesP(color_dst, linesP, 1, CV_PI / 180, 50, 180, 10); // runs the actual detection
    ...
    for (size_t i = 0; i < linesP.size(); i++) {
        Vec4i l = linesP[i];
        //On trace tous les segments parallèles inférieurs à 45 degrése (valeur 1)
    ...
    std::vector<Point> p2;
    p2 = unification(unification(unification(p1,10,18),10,18),10,18);

    std::cout << "Nombre de marches: " << p2.size() << "\n";
    
    for (int j = 0; j < p2.size(); j++) {
        //std::cout << "point " << j << " :" << p2[j] << " " << "\n";
    ...
    std::vector<Point> p3;
    Mat points2 = cv::Mat::zeros(color_dst.rows, color_dst.cols, CV_8UC1);
    ...
    //std::cout << "Points entre les 2 dernieres marches: " << direction <<"\n";
    if(direction == 0)
